...
partInit.code() += "exception |= (*(system->dread))(system->handle, cpu, " + std::to_string(ic.current_address_) + ", (etiss_uint8*)&mem_val_1, 4);\n";
...
partInit.code() += "*((RV32IMACFD*)cpu)->X[" + std::to_string(10U) + "] = etiss_semihost(cpu, system, plugin_pointers, " + std::to_string(32) + ", operation, parameter);\n";
partInit.code() += "}\n";
partInit.code() += " else {\n";
